// React Bits Pixel Blast source: https://reactbits.dev/backgrounds/pixel-blast
// Upstream MIT + Commons Clause: docs/licenses/react-bits.txt.
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    const resume = () => {
      motionAllowedRef.current = !media.matches && !document.hidden;
      ...
      if (!motionAllowedRef.current) {
        ...
        t.clock.reset();
        t.raf = requestAnimationFrame(animationRef.current);
      }
    };
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (
          ...
          threeRef.current &&
          !threeRef.current.raf
        ) {
          threeRef.current.clock.reset();
          threeRef.current.raf = requestAnimationFrame(animationRef.current);
        }
      },
      { threshold: 0 },
    );
    if (autoPauseOffscreen) observer.observe(container);
